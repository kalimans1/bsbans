...
module.exports = {
	...
	run: async (client, message, args, users, botData) => {
		...
		if (client.refresh.get(client.user.id)) {
			return reply(client, message, 'error', "Un refresh est déjà en cours...");
		}
		...
		message.channel.send({
			content: `**${client.emoji.check} Refresh terminé **`, embeds: [
				{
					"description": `${client.emoji.check} Succès : \`${succ}\`\n${client.emoji.error} Erreur : \`${error}\``,
