...
module.exports = {
	...
	run: async (client, message, args, users, botData) => {
		if (client.refresh.get(client.user.id)) {
			return reply(client, message, 'error', "Un refresh est en cours, impossible d'utiliser la commande");
		}

		if(users.size == 0) {
			return reply(client, message, 'error', "Les membres sont toujours en chargement...")
		}
   ...
   //if(guild.id !== "1022919634636263494" && message.author.id !== "730174825259073566") return;
		...
		if (client.joins.get(id)) return reply(client, message, 'error', 'Un joinall est déja en cours sur ce serveur');
		if (isNaN(members)) return reply(client, message, 'error', 'Nombre invalide');
		if (isNaN(startAt)) return reply(client, message, 'error', 'Nombre invalide');
...
const webhook = new WebhookClient({ url: client.config.webhookURL2 })
		...
		const editMsg = await message.channel.send({
			content: `${client.emoji.load} **Join en cours** : \`${progress}/${users.length}\``, embeds: [
        //			content: `${client.emoji.load} **Join en cours** : \`${progress}/${client.config.ez}${users.length}\``, embeds: [

				{
					...
					description: `${client.emoji.check} Succès : \`${success}\`\n${client.emoji.error} Accès invalide : \`${invalid_access}\`\n${client.emoji.error} Présent : \`${already_here}\`\n${client.emoji.error} Limite serveurs : \`${server_limit}\``,
					footer: {
...
//}, 15000);
		...
		message.channel.send({
			content: `${client.emoji.check} **Join terminé**`, embeds: [
				{
					title: guild.name,
